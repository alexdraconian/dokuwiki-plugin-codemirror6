...
async function main() {
    ...
    const originalTabValue = await evaluate("window.__dokuWikiCodeMirror6.editor.port.readValue()");
    const tabResult = await evaluate("(() => { const i = window.__dokuWikiCodeMirror6; const view = i.editor.adapter.editor.view; i.editor.port.writeValue('one\\ntwo\\nthree'); view.dispatch({selection: {anchor: 0, head: 7}}); view.focus(); const tabEvent = new KeyboardEvent('keydown', {key: 'Tab', code: 'Tab', bubbles: true, cancelable: true}); view.contentDOM.dispatchEvent(tabEvent); const tabValue = i.editor.port.readValue(); i.editor.port.writeValue('  one\\n  two\\nthree'); view.dispatch({selection: {anchor: 0, head: 10}}); const shiftTabEvent = new KeyboardEvent('keydown', {key: 'Tab', code: 'Tab', shiftKey: true, bubbles: true, cancelable: true}); view.contentDOM.dispatchEvent(shiftTabEvent); return {tabValue, shiftTabValue: i.editor.port.readValue(), tabPrevented: tabEvent.defaultPrevented, shiftTabPrevented: shiftTabEvent.defaultPrevented, setting: i.editor.settings.get('tabsize'), tabSize: view.state.tabSize}; })()");
    assert(tabResult.tabValue === "  one\n  two\nthree", "browser Tab handling indents every selected line with spaces");
    assert(tabResult.shiftTabValue === "one\ntwo\nthree", "browser Shift+Tab removes one indentation level from selected lines");
    assert(tabResult.tabPrevented && tabResult.shiftTabPrevented && tabResult.setting === "2" && tabResult.tabSize === 2, "browser editor captures Tab and Shift+Tab with the default two-space setting");
    await evaluate("(() => { const i = window.__dokuWikiCodeMirror6; i.editor.port.writeValue(" + JSON.stringify(originalTabValue) + "); i.editor.port.syncForSubmit(); return true; })()");
    ...
    assert(menu.items >= 60 && menu.dracula && menu.groups === 4, "settings menu renders nested theme/font/keymap/tab-size groups");
